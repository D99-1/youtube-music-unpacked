...
for (let i = 0; i < watchHistory.length; i++) {
    if (watchHistory[i].header === 'YouTube Music') {
       id = watchHistory[i].titleUrl.split("https://www.youtube.com/watch?v=").pop().split("&")[0]
        newdata ={
...
let mostPlayed = []
for(let i=0; i<ytm.length;i++){
    newdata = ytm[i].title + " by " + ytm[i].artist
...
for(let i = 0; i<5;i++){
 ...
 //   downloadImage(`
   // https://img.youtube.com/vi/${ytm.find(JSON.stringify(item => item.title === Object.keys(top5)[i])).titleUrl.split("https://music.youtube.com/watch?v=").pop().split("&")[0]}/maxresdefault.jpg`,
     //   __dirname+`/temp/image${[i]}.png`)
}

let mostPlayedArtist = []
for(let i=0; i<ytm.length;i++){
    newdata = ytm[i].artist 
    mostPlayedArtist.push(newdata)
}
let topMostPlayedArtist = {}
mostPlayedArtist.forEach(element => {
    topMostPlayedArtist[element] = (topMostPlayedArtist[element] || 0) + 1;
  });

  
const top5Artist = pickHighest(topMostPlayedArtist,5)
...
let songsListened = ObjectLength(ytm)
let uniqueSongs = Object.keys(topMostPlayed).length
let timeListened = (songsListened * 3.87)



fullStatsData = [{"songsListened":songsListened},{"uniqueSongs":uniqueSongs},{"timeListened":timeListened},{"year":yearlyListensTotal},{"month":monthlyListensTotal},{"top5":top5}]
...
console.log(top5)
console.log(top5Artist)
console.log({songsListened,uniqueSongs,timeListened})
